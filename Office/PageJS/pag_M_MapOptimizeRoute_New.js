...
function ShowMap(optimize)
{
	...
	var startPlan = new google.maps.LatLng( '3.126137', '101.617067' );
	var endPlan = new google.maps.LatLng( '3.137250', '101.613021' );
	
	var wayptsPlan = [];
	
	mapRoutePlanSeq = [{"LATITUDE":"3.128519","LONGITUDE":"101.626886"}, {"LATITUDE":"3.145035","LONGITUDE":"101.623177"}, {"LATITUDE":"3.130505","LONGITUDE":"101.634883"}];
	
	var lengthPlan = mapRoutePlanSeq.length;
	for (var rowPlan = 0; rowPlan < lengthPlan; rowPlan++)
	{
		wayptsPlan.push({
			location:new google.maps.LatLng(mapRoutePlanSeq[rowPlan].LATITUDE,mapRoutePlanSeq [rowPlan].LONGITUDE),
			stopover:true
		});
	}
	...
	directionsServicePlan.route(requestPlan,function(response, status) {
		if (status == google.maps.DirectionsStatus.OK) {
			...
			for(var i = 0; i < waypoint_order.length ; i++ ){
				var newOrder = waypoint_order[i];//zero based
				var wayPoint = mapRoutePlanSeq[ newOrder ];
